...
entity gpu_videoout_async is
   ...
   (
      ...
      dbg_pixeldata_nonnavy   : out std_logic;
      dbg_srcpix              : out std_logic_vector(31 downto 0);  -- [31:21]=readAddr [20]=lineIn(0) [15:0]=displayed source pixel
      dbg_srcxor              : out std_logic_vector(31 downto 0)   -- [31:16]=READ16 count [15:0]=xor of displayed source pixels
   );
end entity;
...
architecture arch of gpu_videoout_async is
   ...
   -- DIAGNOSTIC: capture the displayed source pixel exactly at READ16 (the real display read)
   signal dbg_srcpix_last     : std_logic_vector(31 downto 0) := (others => '0');
   signal dbg_srcpix_xor      : std_logic_vector(15 downto 0) := (others => '0');
   signal dbg_srcpix_cnt      : unsigned(15 downto 0) := (others => '0');
...
begin
   ...
   -- DIAGNOSTIC: pixelData_R/G/B latched after READ16; navy = (R=0, G=0, B=0x84)
   -- '1' on any cycle where the RGB output differs from pure-navy.
   dbg_pixeldata_nonnavy <= '1' when (pixelData_R /= x"00" or pixelData_G /= x"00" or pixelData_B /= x"84") else '0';
   dbg_srcpix <= dbg_srcpix_last;
   dbg_srcxor <= std_logic_vector(dbg_srcpix_cnt) & dbg_srcpix_xor;
      ...
      if rising_edge(clkvid) then
         ...
         if (reset = '1') then
            ...
            if (nextHCount > 1) then
               ...
               if (isVsync = '0') then
                  if (videoout_settings.GPUSTAT_VerRes = '1') then
                     if (videoout_reports.interlacedDisplayField = '1') then
                        lineIn <= to_unsigned((((vdispNew - vDisplayStart) * 2) + 1) mod 512, 9);
                     else
                        lineIn <= to_unsigned(((vdispNew - vDisplayStart) * 2) mod 512, 9);
                     end if;
                  else
                     lineIn <= to_unsigned((vdispNew - vDisplayStart) mod 512, 9);
                  end if;
               end if;
                  ...
                  if (vdispNew = vtotal) then
                     if (rotate180 = '1') then
                        if (videoout_settings.GPUSTAT_VerRes = '1' and interlacedDisplayFieldMuxed = '1') then
                           nextLineCalc := to_unsigned((lineMax - 2) mod 512, 9);
                        else
                           nextLineCalc := to_unsigned((lineMax - 1) mod 512, 9);
                        end if;
                     else
                        if (videoout_settings.GPUSTAT_VerRes = '1' and interlacedDisplayFieldMuxed = '1') then
               ...
               if (rotate180 = '1') then
                  videoout_request.lineInNext <= to_unsigned((lineMax - 1) mod 512, 9) - nextLineCalc;
               else
         ...
         if (reset = '1') then
            ...
            case (state) is
               ...
               when WAITNEWLINE =>
                  ...
                  nextLineCalc := to_unsigned((lineMax - 1) mod 512, 9) - lineIn;
               ...
               when DRAW =>
                  ...
                  case (readstate) is
                     ...
                     when READ16 =>
                        ...
                        ditherCE           <= '1';
                        -- DIAGNOSTIC: this is the actual display read of a source pixel
                        dbg_srcpix_last    <= std_logic_vector(videoout_readAddr) & lineIn(0) & "0000" & videoout_pixelRead;
                        dbg_srcpix_xor     <= dbg_srcpix_xor xor videoout_pixelRead;
                        dbg_srcpix_cnt     <= dbg_srcpix_cnt + 1;
                        if (rotate180 = '1') then
                           videoout_readAddr  <= videoout_readAddr - 1;
                        else
                           videoout_readAddr  <= videoout_readAddr + 1;
