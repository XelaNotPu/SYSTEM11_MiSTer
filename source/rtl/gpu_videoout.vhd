...
entity gpu_videoout is
   ...
   (
      ...
      requestVRAMYPos            : out unsigned(9 downto 0);  -- 10-bit: full 2MB VRAM Y (was 9-bit display-only)
      ...
      dbg_videoout_linebuf_nonnavy   : out std_logic;
      dbg_videoout_pixeldata_nonnavy : out std_logic;
      dbg_dispread                   : out std_logic_vector(31 downto 0);  -- VRAM-readback pixel (mode D): VRAM[ov_x,ov_y]
      dbg_linebufrd                  : out std_logic_vector(31 downto 0);  -- line-buffer OUTPUT (videoout_pixelRead) xor&last
      dbg_vram_coord                 : in  std_logic_vector(31 downto 0) := (others => '0')  -- JTAG VRAM-read: [31:28]=F enable, [19:10]=Y, [9:0]=X
   );
end entity;
...
architecture arch of gpu_videoout is
   ...
   signal reqPosY             : unsigned(9 downto 0) := (others => '0');  -- 10-bit: full 2MB VRAM Y

   -- JTAG VRAM-readback override
   signal ov_active           : std_logic := '0';
   signal ov_x                : unsigned(9 downto 0) := (others => '0');
   signal ov_y                : unsigned(9 downto 0) := (others => '0');
   signal ov_pix              : std_logic_vector(31 downto 0) := (others => '0');
   ...
   -- diagnostic latches: capture line-buffer input vs output to localize corruption
   signal dbg_dispread_xor    : std_logic_vector(15 downto 0) := (others => '0');
   signal dbg_dispread_last   : std_logic_vector(15 downto 0) := (others => '0');
   signal dbg_lbrd_xor        : std_logic_vector(15 downto 0) := (others => '0');
   signal dbg_lbrd_last       : std_logic_vector(15 downto 0) := (others => '0');
   signal dbg_srcpix_sig      : std_logic_vector(31 downto 0);  -- displayed source pixel (mode D)
   signal dbg_srcxor_sig      : std_logic_vector(31 downto 0);  -- displayed-pixel count+xor (mode E)
...
begin 
   ...
   (
      ...
      dbg_pixeldata_nonnavy   => dbg_videoout_pixeldata_nonnavy,
      dbg_srcpix              => dbg_srcpix_sig,
      dbg_srcxor              => dbg_srcxor_sig
   );

   -- DIAGNOSTIC (build #5): write-side fill tap — was the dpram actually written with non-navy
   -- VRAM data this cycle? Fires only on the line-buffer fill cycle (vram_DOUT_READY AND store|store2)
   -- so it can't be triggered by stale random dpram contents or addresses outside the fill window.
   dbg_videoout_linebuf_nonnavy <= '1' when (vram_DOUT_READY = '1' and (store = '1' or store2 = '1') and
   ...
   -- JTAG VRAM-READBACK (mode D): when dbg_vram_coord[31:28]=0xF, the videoout fetches VRAM[ov_x,ov_y]
   -- on every line (reqPosX/Y forced) and ov_pix latches the first fetched word = VRAM[ov_x..ov_x+1, ov_y].
   dbg_dispread  <= ov_pix;             -- mode D = VRAM[ov_x,ov_y] (2 pixels: [15:0], [31:16])
   dbg_linebufrd <= dbg_srcxor_sig;     -- mode E = displayed-pixel count+xor

   process (clk2x)
   begin
      if rising_edge(clk2x) then
         -- latch the JTAG read coordinate (sticky until next 0xF write)
         if (dbg_vram_coord(31 downto 28) = x"F") then
            ov_active <= '1';
            ov_x      <= unsigned(dbg_vram_coord( 9 downto 0));
            ov_y      <= unsigned(dbg_vram_coord(19 downto 10));
         end if;
         -- capture the first fetched word of a forced (ov_x,ov_y) read (fillAddr at line base)
         if (ov_active = '1' and vram_DOUT_READY = '1' and (store = '1' or store2 = '1')
             and fillAddr(7 downto 0) = x"00") then
            ov_pix <= vram_DOUT(31 downto 0);
         end if;
      end if;
   end process;

   -- OUTPUT tap on clkvid: what the line buffer reads back for display (the selected pixel source)
   process (clkvid)
   begin
      if rising_edge(clkvid) then
         dbg_lbrd_last <= videoout_pixelRead;
         dbg_lbrd_xor  <= dbg_lbrd_xor xor videoout_pixelRead xor videoout_pixelRead2;
      end if;
   end process;
            ...
            case (state) is
               ...
               when WAITREQUEST => 
                  if (waitcnt > 0) then
                     ...
                     if (ov_active = '1') then    -- JTAG VRAM-readback: force every fetch to (ov_x,ov_y)
                        reqPosX <= ov_x;
                        reqPosY <= ov_y;
                     else
                        reqPosX <= DisplayOffsetX;
                        reqPosY <= resize(videoout_request_clk2x.lineInNext, 10) + resize(DisplayOffsetY, 10);
                     end if;
   ...
   (
      ...
      BACKGROUNDON           => '1',
      RGB_BACK               => x"FFFFFF",
      RGB_FRONT              => x"FF0000",  -- 2026-07-06: BLUE digits (channel order BBGGRR)
      OFFSETX                => 4,
      OFFSETY                => 4
   )
   port map
